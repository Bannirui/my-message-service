...
public class KafkaConsumerProxy extends ConsumerProxy<ConsumerRecord<String, byte[]>> {
    ...
    protected void consumerStart() {
        ...
        this.kafkaProperties.put("client.id", this.metadata.getName() + "--" + MmsConst.MMS_IP + "--" + ThreadLocalRandom.current().nextInt(100000));
                            ...
                            label196:
                            {
                                label197:
                                {
                                    try {
                                        ...
                                        if (var13) {
                                            for (boolean offerx = this.acks.offer(consumerRecord); !offerx;
                                                 offerx = this.acks.offer(consumerRecord)) {
                                                logger.info("add consumer record to acks full and trigger commit offsets at {}", LocalDateTime.now());
                                                this.commitOffsets();
                                            }
                                            countDownLatch.countDown();
                                ...
                                label249:
                                {
                                    label262:
                                    {
                                        label263:
                                        {
                                            try {
                                                succ = true;
                                                if (!KafkaConsumerProxy.super.msgFilter(KafkaConsumerProxy.this.getMqTagValue(consumerRecord))) {
                                                    succ = false;
                                                    break label249;
                                                }
