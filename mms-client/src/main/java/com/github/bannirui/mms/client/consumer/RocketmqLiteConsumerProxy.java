...
public class RocketmqLiteConsumerProxy extends MmsConsumerProxy<MessageExt> {
    ...
    private final ScheduledExecutorService
        scheduledExecutorService = Executors.newSingleThreadScheduledExecutor((r) -> new Thread(r, "MmsRocketMqLiteConsumerScheduledThread"));
    ...
    public RocketmqLiteConsumerProxy(ConsumerGroupMetadata metadata, boolean order, String instanceName, Set<String> tags, Properties properties, MessageListener listener) {
    ...
    protected void consumerStart() {
        ...
        if (((ConsumerGroupMetadata)this.metadata).needSuspend()) {
            ...
            } else if (com.github.bannirui.mms.client.common.ConsumeFromWhere.EARLIEST.getName().equalsIgnoreCase(consumeFrom)) {
    ...
    private void allocate(String topic, Set<MessageQueue> mqDivided, String consumeFrom) {
        try {
            ...
            if (!topic.startsWith("%RETRY%")) {
                ...
                for (MessageQueue mq : mqDivided) {
                    ...
                    try {
                        ...
                        if (offset == null || offset < 0L) {
                            ...
                            if (com.github.bannirui.mms.client.common.ConsumeFromWhere.EARLIEST.getName().equalsIgnoreCase(consumeFrom)) {
                ...
                for (MessageQueue mq : mqDivided) {
                    if(this.lastRetryAssignSet.contains(mq)) continue;
                    logger.info("MessageQueue: brokerName:{},topic:{},queueId:{}被加入", mq.getBrokerName(), mq.getTopic(), mq.getQueueId());
                    try {
                        offset = this.consumer.committed(mq);
                        if (offset == null || offset < 0L) {
    ...
    private RocketmqLiteConsumerProxy.ConsumeMessageService createConsumeMessageService(MessageQueue messageQueue) {
        return super.isOrderly ? new ConsumeMessageOrderlyService(messageQueue) : new ConsumerMessageConcurrentlyService(messageQueue);
    }
    ...
    private void setClientId() {
        ...
        if (this.metadata.isGatedLaunch()) {
            ...
            this.consumer.setClientIP("consumer-client-id-" + this.metadata.getGatedCluster().getClusterName() + "-" + MmsConst.MMS_IP + "-" + now);
        } else {
            this.consumer.setNamesrvAddr(this.metadata.getClusterMetadata().getBootAddr());
            this.consumer.setClientIP("consumer-client-id-" + this.metadata.getClusterMetadata().getClusterName() + "-" + MmsConst.MMS_IP + "-" + now);
        }
    }
